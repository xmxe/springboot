...
public class TransactionAop {
	...
	/**
	 * 用计数1初始化的mainDownLatch当作一个简单的开/关锁存器，或入口:在通过调用countDown()的线程打开入口前，所有调用await的线程都一直在入口处等待。
	...
	@Around("@annotation(com.xmxe.anno.SonTransaction)")
	public Object sonIntercept(ProceedingJoinPoint joinPoint) throws Throwable {
		...
		if (mainDownLatch == null) {
			// 主事务未加注解时, 直接执行子事务
			return joinPoint.proceed();//这里最好的方式是:交由上面的thread来调用此方法,但我没有找寻到对应api,只能直接放弃事务, 欢迎大神来优化, 留言分享
		}
		...
		try {
			...
			mainDownLatch.await();
			// 如果能执行到这一步说明所有子线程都已经执行完毕 判断如果atomicBoolean是true就回滚false就提交
			if (rollBackFlag.get()) {
