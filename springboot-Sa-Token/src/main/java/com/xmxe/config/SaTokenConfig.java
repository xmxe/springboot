...
//			List<String> ignoreUrls = ignoreUrlsConfig.getUrls();
			...
			SaRouter.match("/brand/listAll", (r) -> {
				StpUtil.checkRoleOr("ROLE_ADMIN","ROLE_USER");
				//强制退出匹配链
				SaRouter.stop();
			});
			SaRouter.match("/brand/**", (r) -> StpUtil.checkRole("ROLE_ADMIN"));
			// StpUtil.checkRoleOr("super-admin", "shop-admin"); 当前账号是否含有指定角色标识 [指定多个，只要其一验证通过即可]

			// 权限认证
			SaRouter.match("/brand/list", (r) -> StpUtil.checkPermission("brand:read"));
			SaRouter.match("/brand/{id}", (r) -> StpUtil.checkPermission("brand:read"));
		})).addPathPatterns("/**");
	}
